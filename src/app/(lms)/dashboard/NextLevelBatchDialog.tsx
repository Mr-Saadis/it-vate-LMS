...
import { X, Calendar, CheckCircle2, AlertCircle, Clock, ArrowRight, Loader2 } from 'lucide-react'
import { format, parseISO } from 'date-fns'
import { enrollNextLevel } from '@/lib/actions/courses'
import { toast } from 'sonner'

export interface NextLevelBatchInfo {
...
interface NextLevelBatchDialogProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  /** The next level to enroll into */
  levelId: string
  levelNo: number
  levelTitle: string
  courseId: string
  trackType: string
  /** Available batches for this level */
...
export function NextLevelBatchDialog({
  isOpen,
  onClose,
  onSuccess,
  levelId,
  ...
  const handleConfirm = async () => {
    if (batches.length > 0 && !selectedBatchId) return
    setIsSubmitting(true)

    const result = await enrollNextLevel(
      levelId,
      selectedBatchId,
      trackType,
      courseId
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/50 mb-0.5">
                {trackType} Track — Next Level
              </p>
              <h3 className="text-sm font-bold text-white leading-tight">
                Select Batch for Level {levelNo}
              </h3>
            </div>
          </div>
          {!isSubmitting && (
        ...
        {/* Level Info Strip */}
        <div className="px-6 py-3 border-b border-slate-100 bg-[#0F172A]/[0.03] flex items-center gap-3">
          <div className="h-8 w-1 rounded-full bg-[#F18231]" />
          <div>
          ...
          <div className="ml-auto">
            <span className="inline-flex items-center gap-1 rounded-md bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700 uppercase tracking-wider">
        ...
        {submitted ? (
          <div className="flex flex-col items-center gap-4 px-6 py-12 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-50 border border-emerald-200 animate-in zoom-in duration-300">
              <CheckCircle2 className="h-8 w-8 text-emerald-500" />
            </div>
            <div>
              <h4 className="text-sm font-bold text-[#0F172A] mb-1">Enrollment Request Submitted!</h4>
              <p className="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
                Your request for Level {levelNo} is pending admin approval. You'll gain access once approved.
              </p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="px-6 py-5 max-h-[380px] overflow-y-auto">
              <p className="text-xs text-slate-500 mb-4 leading-relaxed">
                Your payment for this level is already covered. Choose a batch to join — enrollment will activate after admin approval.
              </p>
                ...
                <div className="flex flex-col items-center gap-3 py-8 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
                      No scheduled batches for this level right now. You can still proceed and a batch will be assigned later.
                    </p>
                  </div>
                </div>
              )}
                  ...
                  {batches.map((batch) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => setSelectedBatchId(batch.content_items_id)}
                        className={`relative rounded-xl border-l-[3px] border p-4 cursor-pointer transition-all duration-150 ${
                          isSelected
                            ? 'border-l-[#F18231] border-slate-200 bg-[#F18231]/5 ring-2 ring-[#F18231]/15'
            ...
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between gap-3">
              <button
                ...
                className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>Proceed Without Batch <ArrowRight className="h-3.5 w-3.5" /></>
                ) : selectedBatchId ? (
                  <>Confirm & Enroll <ArrowRight className="h-3.5 w-3.5" /></>
                ) : (
            ...
            <div className="px-6 py-2.5 bg-[#0F172A]/[0.03] border-t border-slate-100 flex items-center gap-2">
              ...
              <p className="text-[10px] text-slate-500 font-medium">
                No payment required — this level is already covered by your existing enrollment. Activation requires admin approval.
              </p>
            </div>
          </>
        )}
